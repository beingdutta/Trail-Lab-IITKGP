            ...
            <form id="add-form" class="grid gap-4 bg-slate-50 dark:bg-slate-900 p-4 rounded-lg border border-slate-200 dark:border-slate-700">
                ${schema.map(field => `
                    <div>
                        ...
                        ${field.type === 'textarea' 
                            ? `<textarea name="${field.key}" class="w-full p-2 rounded border dark:bg-slate-800 dark:border-slate-600" rows="3" required></textarea>`
                            : field.type === 'select'
                                ? `<select name="${field.key}" class="w-full p-2 rounded border dark:bg-slate-800 dark:border-slate-600">${field.options.map(o => `<option value="${o}">${o}</option>`).join('')}</select>`
                                : `<input type="${field.type}" name="${field.key}" class="w-full p-2 rounded border dark:bg-slate-800 dark:border-slate-600" required>`
                        }
                    </div>
                `).join('')}
            ...
            ${items.map(item => `
                <div class="flex justify-between items-center p-3 bg-slate-50 dark:bg-slate-900 rounded border border-slate-200 dark:border-slate-700">
                    <div class="truncate pr-4">
                        <span class="font-bold">${item[schema[0].key]}</span>
